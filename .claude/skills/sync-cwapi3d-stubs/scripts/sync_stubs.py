...
def build_report(
    ...
    for module, stub in stubs.controllers.items():
        ...
        for name in sorted(stub.functions - bound_by_module.get(module, set())):
            report.orphans.append(Gap(kind='function', module=module, name=name, detail='no C++ binding'))
    ...
    if not only or 'cadwork' in only:
        ...
        for entry in inventory.types:
            ...
            if entry.python_name in config.blacklist_types:
                report.blacklisted.append(Gap(kind='type', module='cadwork', name=entry.python_name))
                continue
            if names & known:
                continue
...
def apply_changes(
    ...
    for module in sorted(new_modules):
        ...
        page = _emit.write_docs_page(config.docs_dir, module, _emit._title_case(module), module)
        report.written.append(str(page.relative_to(config.stub_repo)))
        if _emit.patch_mkdocs_nav(config.mkdocs, module, _emit._title_case(module), 'Reference'):
    ...
    wanted = {(gap.module, gap.name) for gap in report.missing if gap.kind == 'function'}
    for binding in inventory.bindings:
        ...
        if trampoline is None:
            report.warnings.append(f'{binding.module}.{binding.python_name}: no signature found -- skipped')
            continue
        doc = doxygen.lookup(trampoline.accessor, trampoline.cpp_method)
        rendered = _emit.render_function(binding, trampoline, doc, resolver, config.param_names)
        ...
        if rendered.thin_doc:
            report.warnings.append(
                f'{binding.module}.{binding.python_name}: no Doxygen @brief -- docstring is a placeholder'
            )
    ...
    for module, blocks in sorted(by_module.items()):
        ...
        _stubs.append_block(stub.path, separator.join(blocks), stub.blank_lines_between_defs)
    ...
    for entry in inventory.types:
        ...
        if entry.kind == 'enum':
            page = _emit.append_to_enums_page(config.docs_dir, config.enums_page, entry.python_name)
            report.written.append(str(page.relative_to(config.stub_repo)))
        else:
    ...
    if resolver.unresolved:
        report.warnings.append(
            'C++ types with no Python mapping (annotated Any): ' + ', '.join(sorted(resolver.unresolved))
        )
    ...
    if touched_src and config.bump_version:
        ...
        if change is not None:
            ...
            report.warnings.append(
                'could not set [project].version -- the publish workflow will reject a duplicate upload'
            )
...
def print_report(report: Report, applied: bool) -> None:
    ...
    if report.blacklisted:
        ...
        suffix = (
            f' (whole module{"s" if len(skipped_modules) > 1 else ""}: {", ".join(skipped_modules)})'
            if skipped_modules
            else ''
        )
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description='Sync cwapi3d .pyi stubs with the CwAPI3D pybind11 bindings.')
    ...
    try:
        ...
        enum_definitions = _cpp_bindings.parse_enum_definitions(list(config.enum_search_dirs))
    ...
    if unknown:
        print(
            'unknown --only module(s): ' + ', '.join(unknown) + '\nknown: ' + ', '.join(sorted(known_modules)),
            file=sys.stderr,
        )
        return EXIT_ERROR
    ...
    if args.apply:
        return EXIT_ERROR if any(warning.startswith('SYNTAX ERROR') for warning in report.warnings) else EXIT_OK
